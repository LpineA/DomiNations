...
public class Personn {
	String pseudo;
	int nbreRois;
	ArrayList<Integer> ordreTour = new ArrayList<Integer>();/// on rajoute ce caracteristique car il nous permet de
															/// savoir quelle joueur pose en premier
	Hashtable<Integer, String[]> Terrain = new Hashtable<Integer, String[]>();

	// pour le moment juste un tableau vide et on initalize la carte du centre a
	// center
		...
		for (int i = 0; i < 81; i++) {
			for (int j = 0; j < 2; j++) {
				if (Terrain.get(i) == null) {
					...
					System.out.println(" - Sur le numéro " + i + " et celle-ci comporte est de type :" + Terrain.get(i)
							+ " ; et celle-ci comporte " + Terrain.get(i) + " rois ! ");
				}
			}
	...
	public void setPositionofDominos(Card Card) {
		...
		// -----------------------Initilisation des variables de la
		// carte------------------------//
		...
		// ----------------------Fin de l'initialisaton des variables de la
		// carte------------------//
		...
		// ----------------------Vérification du bon positionnement des dominos dans un
		// tableau de 5 par 5------------------------//
		...
		for (int j = 9; j < 18; j++) {
			if (Terrain.get(j)[0] != null && caseFace1 > 54 && checkIfElementInLine == false
					|| Terrain.get(j)[0] != null && caseFace2 > 54 && checkIfElementInLine == false) {
				System.out.println(
						"Votre terrain ne doit pas dépasser une taille de 5 x 5, veuillez donc réésayer de positionner votre domino ! ");
				setPositionofDominos(Card);
			}
		}
		...
		for (int j = 63; j < 72; j++) {
			if (Terrain.get(j)[0] != null && caseFace1 < 27 && checkIfElementInLine == false
					|| Terrain.get(j)[0] != null && caseFace2 < 27 && checkIfElementInLine == false) {
				System.out.println(
						"Votre terrain ne doit pas dépasser une taille de 5 x 5, veuillez donc réésayer de positionner votre domino ! ");
		...
		positionToDeleteRight.addAll(Arrays.asList(5, 6, 7, 8, 14, 15, 16, 17, 23, 24, 25, 26, 32, 33, 34, 35, 41, 42,
				43, 44, 50, 51, 52, 53, 59, 60, 61, 62, 63, 68, 69, 70, 71, 80));
		...
		for (int j = 1; j < 81; j = j + 9) {
			if (Terrain.get(j)[0] != null && positionToDeleteRight.contains(caseFace1) == true
					&& checkIfElementInLine == false
					|| Terrain.get(j)[0] != null && positionToDeleteRight.contains(caseFace1) == true
							&& checkIfElementInLine == false) {
				System.out.println(
						"Votre terrain ne doit pas dépasser une taille de 5 x 5, veuillez donc réésayer de positionner votre domino ! ");
				setPositionofDominos(Card);
		...
		positionToDeleteLeft.addAll(Arrays.asList(0, 1, 2, 3, 9, 10, 11, 12, 18, 19, 20, 21, 27, 28, 29, 30, 36, 37, 38,
				39, 45, 46, 47, 48, 54, 55, 56, 57, 63, 64, 65, 66, 72, 73, 74, 75));

		for (int k = 8; k < 81; k = k + 9) {
			if (Terrain.get(k)[0] != null && positionToDeleteLeft.contains(caseFace1) == true
					|| Terrain.get(k)[0] != null && (positionToDeleteLeft.contains(caseFace2) == true)) {
				System.out.println(
						"Votre terrain ne doit pas dépasser une taille de 5 x 5, veuillez donc réésayer de positionner votre domino ! ");
				setPositionofDominos(Card);
			}
		...
		for (int j = 8; j < 81; j = j + 9) {
			if (Terrain.get(j)[0] != null && positionToDeleteLeft.contains(caseFace1) == true
					&& checkIfElementInLine == false
					|| Terrain.get(j)[0] != null && positionToDeleteLeft.contains(caseFace1) == true
							&& checkIfElementInLine == false) {
				System.out.println(
						"Votre terrain ne doit pas dépasser une taille de 5 x 5, veuillez donc réésayer de positionner votre domino ! ");
				setPositionofDominos(Card);
		...
		// ---------------------------------Vérification bon positionnement des
		// dominos----------------------------------------//

		if (caseFace1 == caseFace2 + 1 || caseFace1 == caseFace2 - 1 || caseFace1 == caseFace2 + 9
				|| caseFace1 == caseFace2 - 9) {
			...
			System.out.println(
					"Veuillez entrer une case entre 1 et 81 correspondant au emplacement sur le plateau de jeu ! ");
			setPositionofDominos(Card);
		}
	...
	public void displayTerrainWithKings() {
		...
		for (int i = 0; i < 81; i++) {
			...
			System.out.print("__");
			try {
				System.out.print("" + Terrain.get(i)[1] + "_");
			} catch (NullPointerException e) {
				System.out.print("null");
			}
			System.out.print("__");
	...
	public boolean affirmation(int i) {
		...
		try {
			condition = ((Terrain.get(i))[0] == Terrain.get(i - 1)[0] || Terrain.get(i)[0] == Terrain.get(i + 1)[0]
					|| Terrain.get(i)[0] == Terrain.get(i + 9)[0] || Terrain.get(i)[0] == Terrain.get(i - 9)[0]);
		}
	 ...
	 * int result = 0;
	 * 
	 * try{ if (Terrain.get(i)[0] != null) { if ( affirmation(i) == true &&
	 * affirmation (i-1) == false && affirmation(i+1) == true ){ result = 2; return
	 * result; }
	 * 
	 * else if(affirmation(i) == true && affirmation(i+1) == false ) { result = 1;
	 * return result;
	 * 
	 * 
	 * 
	...
	public Integer counter(int i) {
		try {
			...
			if ((Terrain.get(i)[0] == Terrain.get(i + 1)[0] || (Terrain.get(i)[0] == Terrain.get(i + 9)[0])) == true) {
				...
				/*
				 * if ( (Terrain.get(i)[0] == Terrain.get(i-1)[0] ) && (Terrain.get(i)[0] !=
				 * Terrain.get(i+1)[0]) && (Terrain.get(i)[0] != Terrain.get(i+9)[0]) ) {
				 * nbOfKing =+ Integer.parseInt(Terrain.get(i)[1]); counter++; }
				 */

				/*
				 * if ((Terrain.get(i)[0] == Terrain.get(i+9)[0] ) && (Terrain.get(i)[0] !=
				 * Terrain.get(i+1)[0]) && (Terrain.get(i)[0] != Terrain.get(i+10)[0])) {
				 * counter++; }
				 * 
				 * if( (Terrain.get(i)[0] == Terrain.get(i+1)[0]) && (Terrain.get(i)[0] ==
				 * Terrain.get(i+9)[0]) && Terrain.get(i)[0] != Terrain.get(i+10)[0]) {
				 * counter++; }
				 */
	...
	public void deleteSimpleElements() {
		...
		for (int j = 0; j < 6; j++) {
			...
			for (int i = 0; i < 81; i++) {
				try {
					if (Terrain.get(i)[0] != Terrain.get(i - 1)[0] && Terrain.get(i)[0] != Terrain.get(i + 1)[0]
							&& Terrain.get(i)[0] != Terrain.get(i + 9)[0]
							&& Terrain.get(i)[0] != Terrain.get(i - 9)[0]) {
	...
	public void setPositionOfCastle() {
		...
		System.out.println("Le château sera placé par défaut sur le case centrale ! ");

		String Carte1[] = { "Castle", ""};

		Terrain.put(40, Carte1);
